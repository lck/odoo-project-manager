# Odoo Project Manager (OPM)

OPM manages the odoo development workspace using a single configuration file.
It clones repositories, provisions a virtualenv, and generates config and helper scripts.
It also bootstraps the requested Python version when needed, and builds a local `ROOT/wheelhouse/` so the venv can be provisioned offline.
...
```bash
opm /path/to/ROOT/odoo-project.ini --sync-all --create-venv
```
...
```bash
opm /path/to/odoo-project.ini --sync-all --create-venv --root /path/to/NEW-ROOT
```
...
```bash
opm /path/to/odoo-project.ini --sync-all --create-wheelhouse --root /tmp/build-root --dest-root /srv/odoo/myproject
```

Sync without venv provisioning:

```bash
opm /path/to/ROOT/odoo-project.ini --sync-all
```

Build wheelhouse without venv provisioning:

```bash
opm /path/to/ROOT/odoo-project.ini --sync-all --create-wheelhouse
```

Offline venv provisioning from an existing wheelhouse:

```bash
opm /path/to/ROOT/odoo-project.ini --create-venv --reuse-wheelhouse
```

Hard rebuild of the venv:

```bash
opm /path/to/ROOT/odoo-project.ini --sync-all --rebuild-venv
```

If no options are specified, OPM only regenerates config and helper scripts:

```bash
opm /path/to/ROOT/odoo-project.ini
```

## What OPM creates (directory layout)

`ROOT` is the directory containing `odoo-project.ini` (or the path passed via `--root`).

If you pass `--dest-root`, OPM still creates files under `ROOT`, but **paths embedded inside generated files** are based on `DEST_ROOT`.
...
[virtualenv]
python_version = 3.11
# Optional (default: true): when false, OPM will NOT install a managed CPython via `uv python install`.
...
OPM supports a lightweight include mechanism so you can split configuration across multiple INI files (for example a shared `base.ini` plus a local override file).
...
OPM uses Python's **ExtendedInterpolation**, so you can reference values via `${section:option}` (for example `${vars:branch}`).

In addition, OPM injects a set of **runtime variables** (workspace paths) into the INI `DEFAULT` scope, so they are available from *any* section in the INI file:
...
- OPM always *builds* under `ROOT` (filesystem workspace).
...
OPM always generates an `addons_path` for `odoo-server.conf` that includes:
...
By default, OPM keeps repositories as **full clones** (all branches + tags + full history). For very large repositories (especially `odoo/odoo`) you can enable a **shallow, single-branch** workflow.
...
Set `shallow_clone = false` (or remove it) and re-run a sync. If the repo is shallow and/or single-branch, OPM automatically converts it back to full history (unshallow) and widens the `origin` fetch refspec so a subsequent `fetch --all --tags` can pull all remote branches.
...
OPM installs project Python dependencies into the venv only when you pass `--create-venv` (or `--rebuild-venv`).
...
- If the requested Python version is not available on the machine and managed Python is enabled, OPM installs it via `uv python install`.
...
```bash
opm /path/to/odoo-project.ini --create-venv --reuse-wheelhouse
```
...
By default, OPM manages the requested CPython version for you using `uv`.
...
- `managed_python = true` (default): when OPM needs to create `ROOT/venv/`, it ensures the requested `python_version` exists by running `uv python install`.
- `managed_python = false`: OPM **skips** `uv python install` and relies on an already-installed system Python that matches `python_version`. If such Python is not available, venv creation will fail.
...
(including untracked files), OPM aborts. Commit/stash/clean your working tree
before running OPM.

## Generated scripts

OPM generates helper scripts into `ROOT/odoo-scripts/`. All scripts use the generated configuration file (`odoo-configs/odoo-server.conf`) and forward any additional arguments to the underlying command.
