...
OPM manages an Odoo development workspace using an `odoo-project.ini` file.
It clones/updates repositories, optionally provisions a virtualenv and generates config and helper scripts.
It also bootstraps the requested CPython version when needed.
"""
    ...
    for raw in raw_lines:
        ...
        if no_comment.startswith(("-r ", "--requirement ")):
            ...
            if len(parts) == 2:
                ...
                out_lines.append(f"# opm: begin include {include_rel}")
                if include_path in visited:
                    out_lines.append(f"# opm: skipped recursive include {include_rel}")
                else:
                    visited.add(include_path)
                    out_lines.extend(_filter_requirements_file(include_path, ignore_names, visited=visited))
                out_lines.append(f"# opm: end include {include_rel}")
                continue
        ...
        if name and name in ignore_names:
            out_lines.append(f"# opm: skipped (ignored package '{name}'): {raw}")
            continue
...
def compile_all_requirements_lock(
    ...
    lines: list[str] = [
        "# This file is generated by opm.py (DO NOT EDIT).",
...
def build_parser() -> argparse.ArgumentParser:
    epilog = """If no options are specified, OPM only regenerates configs and helper scripts.
...
Examples:
  opm /path/to/odoo-project.ini --sync-all --create-venv
  opm /path/to/odoo-project.ini --sync-all --create-wheelhouse
  opm /path/to/odoo-project.ini --sync-all --create-venv --root /path/to/workspace-root
  opm /path/to/odoo-project.ini --rebuild-venv --reuse-wheelhouse
  opm /path/to/odoo-project.ini --rebuild-venv --reuse-wheelhouse --root /path/to/workspace-root
"""

    parser = argparse.ArgumentParser(
        prog="opm",
        description=f"Odoo Project Manager {__version__}",
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"Odoo Project Manager {__version__}",
